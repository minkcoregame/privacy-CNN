...
class StatusPage(GridLayout):
    def __init__(self, **kwargs):
        ...
        # Called with a message, to update message text in widget
        Clock.schedule_once(self.wait_for_server_reply, 1)
    ...
    def wait_for_server_reply(self, _):

        client_p3app_socket.receive_message()


        chat_app.create_predict_page()
        chat_app.screen_manager.current = 'Predict'


## This page shows the results of prediction
class PredictPage(GridLayout):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        ...
        with open("predict_result.txt","r") as f:
            ...
            for line in data:
                words =line.split(', ')
            #print(words)
            print('Last word:',words[0])
            encrypt_message = words[0]
        ...
        # Add text widget to the layout
        self.add_widget(Button(text=encrypt_message))

        # add join button.
        self.encrypt = Button(text="Next")
        self.encrypt.bind(on_press=self.encrypt_button)
        self.add_widget(self.encrypt)
    ...
    def connect(self, _):
        ...
        # Create chat page and activate it
        chat_app.create_decrypt_page()
        chat_app.screen_manager.current = 'Decrypt'
...
class DecryptPage(GridLayout):
    ...
    def encrypt_button(self, instance):
        ...
        self.close_app = Label(text='Click for end program')
        self.close_app = Button(text='End!')
        self.close_app.bind(on_press=lambda b: chat_app.stop())

        self.add_widget(self.close_app)
...
class ImageApp(App):
    ...
    def create_predict_page(self):
        ...
        screen = Screen(name='Predict')
        screen.add_widget(self.predict_page)
        self.screen_manager.add_widget(screen)

    def create_decrypt_page(self):
        self.decrypt_page = DecryptPage()
        screen = Screen(name='Decrypt')
        screen.add_widget(self.decrypt_page)
        self.screen_manager.add_widget(screen)
